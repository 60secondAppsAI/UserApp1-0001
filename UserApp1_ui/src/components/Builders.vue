...
const loadBuilders = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/builder/builders?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
    ...
    builders.value = data.builders || [];
    totalElements.value = data.totalElements || 0;
    totalPages.value = Math.ceil(totalElements.value / pageSize.value);
    currentPage.value = page;
  } catch (err: any) {
    console.error('Get builders error:', err);
    error.value = err.message || 'Failed to load builders';
  }
...
const nextPage = () => {
  if (currentPage.value < totalPages.value - 1) {
    loadBuilders(currentPage.value + 1);
  }
};
...
const getBuilderIcon = (builder: any) => {
  const icons = ['👤', '👨', '👩', '🧑', '👱', '🧔', '👨‍💼', '👩‍💼', '🧑‍💻', '👨‍🔬'];
  return icons[builder.builderId % icons.length];
};

const viewBuilderDetails = (builder: any) => {
  router.push({ name: 'BuilderDetail', params: { id: builder.builderId } });
};
...
<template>
  <div class="builders-page">
    ...
    <header class="builders-header">
      <div class="header-row">
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              <h1 class="page-title">Builders</h1>
              <div class="builders-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="builders-content">
      <!-- Loading State -->
      <div v-if="isLoading" class="loading-state">
        <div class="loading-spinner-large"></div>
        <p class="loading-text">Loading builders...</p>
      </div>

      <!-- Error State -->
      <div v-else-if="error" class="error-state">
        <div class="error-icon">⚠️</div>
        <h3 class="error-title">Failed to Load Builders</h3>
        <p class="error-message">{{ error }}</p>
        <button @click="loadBuilders()" class="retry-button">
          Try Again
        </button>
      </div>

      <!-- Empty State -->
      <div v-else-if="filteredBuilders.length === 0" class="empty-state">
        <div class="empty-icon">👥</div>
        <h3 class="empty-title">No Builders Found</h3>
        <p class="empty-message">
          {{ searchQuery ? 'No builders match your search criteria.' : 'No builders found in the system.' }}
        </p>
      </div>

      <!-- Builders Grid -->
      <div v-else-if="viewMode === 'cards'" class="builders-grid">
        <div
          v-for="builder in filteredBuilders"
          :key="builder.builderId"
          class="builder-card"
          @click="viewBuilderDetails(builder)"
        >
          ...
          <div class="builder-info">
            <h3 class="builder-name">{{ builder.name || 'Unknown Builder' }}</h3>
            <p class="builder-description">
              Builder ID: {{ builder.builderId }}
            </p>
            
            <div class="builder-metadata">

              <div class="metadata-item">
                <span class="metadata-label">BuilderId:</span>
                <span class="metadata-value">{{ builder.builderId || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Name:</span>
                <span class="metadata-value">{{ builder.name || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Description:</span>
                <span class="metadata-value">{{ builder.description || 'N/A' }}</span>
              </div>
      ...
      <!-- Builders Table -->
      <div v-else-if="viewMode === 'table'" class="builders-table-container">
        <table class="builders-table">
          <thead>
            <tr>

              <th @click="handleSort('builderId')" class="sortable-header">
                BuilderId {{ getSortIcon('builderId') }}
              </th>
          ...
          <tbody>
            <tr v-for="builder in filteredBuilders" :key="builder.builderId" class="builder-row">
              
              
                      <td class="builder-id-cell" @click="viewBuilderDetails(builder)">{{ builder.builderId }}</td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ builder.name }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ builder.description }}</span>
                </div>
              </td>
